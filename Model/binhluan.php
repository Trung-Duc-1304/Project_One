...
function load_all_bl($kyw)
{
    ...
    INNER JOIN taikhoan ON binhluan.idtaikhoan=taikhoan.id INNER JOIN sanpham ON binhluan.idsanpham=sanpham.id WHERE binhluan.status = 1";
    if ($kyw != "") {
...
{
    $sql = "UPDATE binhluan SET status = 0 WHERE id=" . $id;
    pdo_execute($sql);
}
